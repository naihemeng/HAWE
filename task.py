...
class Task(object):
    ...
    def _classfication(self, embedding, labels_np, split_ratio=0.7):
        ...
        if len(labels[0]) == 1:
            clf = LogisticRegression(class_weight='balanced', solver='liblinear', max_iter=3000)
    ...
    def classfication(self, embedding, labels_np, split_ratio=0.7, loop=1):
        ...
        for _ in tqdm(range(loop)):
            tmp_dict = self._classfication(embedding, labels_np, split_ratio)
            f1_micro.append(tmp_dict['f1-micro'])
            f1_macro.append(tmp_dict['f1-macro'])
        f1_micro = np.array(f1_micro)
        f1_macro = np.array(f1_macro) 
        print("average performance. f1-micro|f1-macro: {:.4f} +/- {:.4f}|{:.4f} +/- {:.4f}".format(np.mean(f1_micro), np.std(f1_micro), np.mean(f1_macro), np.std(f1_macro)))
    ...
    def k_precision(self, embedding, lbl_path, k=50):
        ...
        elif 'patent' in lbl_path:
            eval_dict = {
                'precision': k,
                'agent': self._k_precision(embedding, label, k, 1),
            }
        else:
            eval_dict = {
                'precision': k,
